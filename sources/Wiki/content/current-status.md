...
**2026-09-22 공통 UI 통합:** 최신 인벤토리를 기준으로 장비 상세·비교·슬롯·캐릭터 장착 배치를 공통 부품에 연결했다. 어두운 올리브·황동·아이보리 테마와 공용 폰트, 창 표시 순서·입력 차단·일시정지 복원, 저장 성공 후 갱신을 함께 관리한다. 사냥 칙령은 요약 목록과 선택 편집, 검색·변경 항목 필터·고정 저장 영역으로 구성했다. 신규 콘텐츠용 기본 틀과 저장소 규칙·자동 검사를 제공한다. 진행 중인 신규 스킬 기획은 포함하지 않는다. [공통 규칙](../../Docs/Implementation/Shared_UI_Contract.md) · [검증 기록](../../Docs/Implementation/Shared_UI_Validation.md)

English: equipment detail/comparison, slots and character equipment now share presentation owners based on the latest inventory. The olive/brass/ivory theme, fonts, window stack, input/pause restoration and committed-state refresh are centralized. Hunt Edict uses summaries with a selected editor, search, changed-group filtering and fixed save actions. New content has a starter, repository rules and automated ownership checks. Ongoing new skill design is excluded. [Shared contract](../../Docs/Implementation/Shared_UI_Contract.en.md) · [Validation](../../Docs/Implementation/Shared_UI_Validation.en.md)

**2026-09-22 대장간 적용:** 기존 대장장이의 2.8m 상호작용으로 옵션 변경·장착 슬롯 강화·장비 강화 화면을 연다. 실제 장비·저장·계정 재화를 연결했으며 장비 +100, +10·최대 강화, 캐릭터별 슬롯 성장, 계정 공용 작업 칸과 종료 중 시간 정산을 구현했다. 강화석은 일반 재료와 분리되며 일반 전설 분해 10개, 세트가 아닌 고유 효과 장비 15개, 세트 10개를 지급한다. 이전 +5와 품질·투자 기록은 보존하고, 품질 배율은 미강화 기본값에 적용한 뒤 강화 보너스를 더한다. [리소스·규칙·저장 이전](../../Docs/Implementation/Blacksmith_Unity_Integration.md) · [검증 기록과 화면](../../Docs/Implementation/Blacksmith_Validation.md)

English: the blacksmith's existing 2.8 m interaction opens native enchanting, character slot growth and equipment enhancement. Real equipment, saves and account currencies support +100 equipment, +10/max batches, shared workstations and offline job completion. Stones are separate from materials: ordinary legendary salvage gives 10, non-set unique-effect gear 15, and sets 10. Legacy +5 and quality/investment ledgers remain intact; quality multiplies the unenhanced base before the fixed enhancement bonus is added. [Resources, rules and migration](../../Docs/Implementation/Blacksmith_Unity_Integration.en.md) · [Validation and screens](../../Docs/Implementation/Blacksmith_Validation.en.md)
