...
- Exposed Weakness sits on the noise floor.
...
No balance numbers were changed. Tuning is a design decision; what follows is what the measurement suggests.

1. **Exposed Weakness is the weakest.** +0.48% damage is the noise floor and 9 → 7 deaths is small. It needs a direct critical to apply and then lasts 4 seconds on one target. It is the only one of the six that is faint on both axes.
...
The editor menu `HELLSCRIPT/룬 엘리트 밸런스 시뮬레이션 실행` does the same. Tier 10 with 8 seeds took about 70 minutes; tier 25 with 4 seeds about 60.
