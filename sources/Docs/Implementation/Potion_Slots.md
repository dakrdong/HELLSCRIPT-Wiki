...
인벤토리의 캐릭터 장비 아래에 물약 전용 슬롯 3개를 추가하고, 하단에 실제 보유 재화를 표시한다. 물약은 캐릭터별 재고, 재화는 계정 공용 잔액을 읽는다. HTML 시안이 아닌 Unity 플레이 씬의 기존 인벤토리에 연결한 기능이다.

English: [Inventory potion slots and account balances](Potion_Slots.en.md)

## 물약 배치와 소진 후 사용

- 슬롯은 장비 칸과 같은 세로 52·가로 54 크기를 사용한다. 그림은 기존 원본을 보존하면서 표시 영역의 가운데에 맞춘다. 오른쪽 위에는 톱니바퀴가 있다.
- 슬롯을 누르면 ‘지정 물약을 모두 소진 했을 시’ 말풍선이 열린다. 높은 등급·낮은 등급·최근 획득·오래된 획득 중 하나를 선택한다. 체크와 강조색으로 현재 선택을 표시한다.
- ‘물약 변경’에서 보유한 물약을 지정하고 ‘해제’로 빈칸을 만든다. 다른 슬롯에 지정한 물약과 미보유 물약은 선택할 수 없다. 장비를 물약 칸에 놓아도 장착되지 않는다.
- 지정 물약을 먼저 사용하고, 소진되면 **같은 효과**의 남은 물약만 해당 기준으로 고른다. 다른 슬롯의 지정품과 이미 선택한 대체품은 제외하며, 후보가 없으면 사용을 멈춘다.
- 배치는 기존 규칙대로 마을에서 변경한다. 소진 후 기준은 슬롯별·캐릭터별로 저장하고 전투 중에도 바꿀 수 있다. UI를 열거나 기준을 바꾸는 것만으로 물약을 소비하지 않는다.
...
- Unity 6000.6.0f1의 관련 Edit Mode 검사 **122개 통과, 실패·건너뜀 0개**: 소진 순서·중복 배치·잘못된 종류·저장 실패·재접속·전투·훈련 격리·기존 HUD·현지화를 포함한다. [결과 XML](PotionSlotsEvidence/editmode.xml)
- macOS 개발 빌드에서 세로 440×956·가로 956×440·PC 1440×810/1440×900/1680×720과 한국어·영어, 글자 100%·150%의 **20조합**을 통과했다. 실제 uGUI 이벤트·레이캐스트로 선택, 배치 변경, 장비 드롭 거부, 저장 재읽기, HUD 반영을 검사했다. [실행 결과](PotionSlotsEvidence/potion-slots-result.txt)
...
대표 화면: [세로](PotionSlotsEvidence/slots-440x956-ko-100.png) · [가로](PotionSlotsEvidence/slots-956x440-ko-100.png) · [영문 확대 말풍선](PotionSlotsEvidence/policy-440x956-en-150.png) · [가로 확대 말풍선](PotionSlotsEvidence/policy-956x440-ko-150.png) · [재화·코어 스크롤](PotionSlotsEvidence/wallet-scrolled-440x956-en-150.png) · [PC 21:9](PotionSlotsEvidence/slots-1680x720-en-100.png)
