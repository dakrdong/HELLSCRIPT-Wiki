...
| Potions | Stay above the highest skill row in both orientations. A translucent dark olive tray with brass decoration and dividers groups the three bottles. Bottle artwork, cooldown overlays and counts align with each compartment’s horizontal center. |
...
## September 23 potion artwork centering correction

The potion slots were centered, but `Image.preserveAspect` used their artwork's left-aligned `RectTransform.pivot` when fitting tall sprites. Earlier layout checks covered the slots and missed the rendered artwork offset. [GlobalHudView](../../Assets/HELLSCRIPT/Runtime/Presentation/GlobalHudView.cs) now centers the pivots of the bottle image and cooldown overlay. Source PNGs, alpha, metadata, counts and hit areas are preserved.

The regression check measures vertices actually generated by uGUI. Before the fix, the health bottle rendered at x=93.6709 instead of its 45-pixel slot center x=102.5, so the check failed. After the fix, all **64 focused tests passed**, including the three bottle images and overlays at sizes 45, 60 and 90. The 9 shared UI checks and macOS development build also passed.

[Before-fix reproduction](TownHudEvidence/2026-09-23/PotionCenter/before.xml) · [After-fix tests](TownHudEvidence/2026-09-23/PotionCenter/editmode.xml) · [Runtime result](TownHudEvidence/2026-09-23/PotionCenter/runtime.txt) · [Validation scope](TownHudEvidence/2026-09-23/PotionCenter/validation.json)

The macOS player passed the 48 resolution/reading-size/language combinations and additional safe-area checks again. The corrected capture below shows each bottle and count sharing its compartment’s horizontal center. Physical mobile devices were not tested.

![Portrait with centered potion artwork](TownHudEvidence/2026-09-23/PotionCenter/portrait-ko.png)

[Portrait 440×956](TownHudEvidence/2026-09-23/PotionCenter/mobile-portrait-ko.png) · [Landscape](TownHudEvidence/2026-09-23/PotionCenter/landscape-ko.png) · [English 150%](TownHudEvidence/2026-09-23/PotionCenter/portrait-en-large.png)
