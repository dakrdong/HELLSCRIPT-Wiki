...
공통 규격은 정사각형 PNG 원본(1024px 목표, 내장 도구가 반환하는 1254px 원본도 허용), 중앙의 큰 실루엣, 충분한 여백, 글자·숫자·테두리 없는 다크 고딕 회화풍이다. 기존 `EquipmentAtlas.png`와 `Art/GlobalHUD` 아이콘을 직접 확인해 질감과 명암을 맞춘다. 부위와 동작은 실루엣으로 구분하고, 효과 색만 바꾼 복제품을 만들지 않는다. 투명 원본은 네이티브 알파로 생성하고 실제 알파·가장자리·64px 가독성을 검수한다. 크로마키나 사후 배경 제거로 대체하지 않는다.
...
원본 크기를 맞추기 위한 별도 이미지 편집은 요구하지 않습니다. 실제 크기를 기록하고 네이티브 원본을 인계합니다. 화면 표시 크기와 Unity 임포트 설정은 UI 통합 단계에서 정합니다.

No separate pixel editing is required solely to reach the target size. Record the actual dimensions and hand off the native master. Display size and Unity import settings belong to UI integration.
