...
namespace Hellscript
{
    public sealed partial class GameUI
    {
        ...
        public void ShowComparisonPicker()
        {
            ...
            pageRepaint=()=>ShowComparisonPicker();Base("comparison-picker","훈련 A/B 비교","같은 캐릭터와 장비로 설정의 차이를 확인합니다");
        ...
        public void ShowComparisonConditions()
        {
            ...
            ComparisonNote(Loc.F("{0} Lv.{1}\n{2}\n이동: {3} {4:0.#}m\n물약: HP {5:0}% 이하", game.catalog.classNames[(int)game.Combat.Hero.heroClass], game.Combat.EffectiveLevel, run.build.name, BehaviorRules.Movements[(int)run.build.movement], run.build.distance, run.build.potionThreshold),gold);
        ...
        void RefreshComparisonDifference()
        {
            ...
            var changes=BuildEditing.DescribeCombatChanges(game.Comparison.A.build,editing,game.catalog,comparisonHero.heroClass);
        ...
        public void ShowComparisonRecord(TrainingComparisonRecord record)
        {
            ...
            pageRepaint=()=>ShowComparisonRecord(record);Base("comparison-result","훈련 A/B 결과",Loc.F("{0} Lv.{1} · {2} · 같은 기준 장비", game.catalog.classNames[(int)hero.heroClass], hero.level, TrainingNames[frozen.training]),true);
        ...
        {
            var row=Row(content,60);var label=Label(row,title,19,muted);Place(label.rectTransform,8,6,256,48);
            var left=Label(row,a,20,pale,TextAnchor.MiddleRight);Place(left.rectTransform,264,6,176,48);
            var right=Label(row,b??"—",20,gold,TextAnchor.MiddleRight);Place(right.rectTransform,448,6,194,48);
        }
        ...
        {
            pageRepaint=()=>ShowComparisonChanges(record);Base("comparison-changes","A와 B의 변경 조건","캐릭터·장비·시드·시작 배치는 같습니다");
            if(record.changes.Count==0)ComparisonNote("A와 B는 같은 전투 설정입니다. 같은 조건에서 결과가 재현되는지 확인한 비교입니다.",gold);
            foreach(string change in record.changes)ComparisonNote(NamedConditions(change));
            FooterButton(0,1,"비교 결과로",()=>ShowComparisonRecord(record));
        }
        void ShowComparisonChoice(TrainingComparisonRecord record,bool useB)
        {
            var chosen=(useB?record.b:record.a).build;pageRepaint=()=>ShowComparisonChoice(record,useB);Base("comparison-choice",Loc.F("{0} 설정 활용", (useB?"B":"A")),"현재 장비는 유지됩니다");
            var changes=BuildEditing.DescribeCombatChanges(game.Store.Data.Hero.build,chosen,game.catalog,game.Store.Data.Hero.heroClass);
            ComparisonNote("현재 행동과 선택한 설정의 차이를 확인하세요. 슬롯에 저장하거나, 성소의 행동 설계에서 편집안으로 불러와 직접 적용할 수 있습니다.");
            ...
            BigButton(content,"실제 행동 편집안으로 불러오기",()=>{game.ReturnTown();ShowBuild();LoadEditing(chosen);},true);
            FooterButton(0,1,"비교 결과로",()=>ShowComparisonRecord(record));
        }
    }
}
